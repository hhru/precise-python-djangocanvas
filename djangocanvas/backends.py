...
from djangocanvas.utils import get_social_user_model
from djangocanvas.exceptions import FacebookAuthorizationDenied, FacebookAuthorizationError
from djangocanvas.models import Facebook
from djangocanvas.api.facepy import SignedRequest
...
SocialUserModel = get_social_user_model()


class BaseSocialAuthBackend(object):
    def get_user(self, user_id):
        try:
            return SocialUserModel.objects.get(pk=user_id)
        except SocialUserModel.DoesNotExist:
            return None
...
class FacebookAuthBackend(BaseSocialAuthBackend):
    ...
    def get_user_from_signed_request(self, request):
        ...
        try:
            social_user = SocialUserModel.objects.get(social_id=social_id)
        except SocialUserModel.DoesNotExist:
            logger.info(u'Creating a new user (facebook id = {0})'.format(social_id))

            social_user = SocialUserModel.create_facebook_user(request.facebook.signed_request)
            request.social_data = social_user.get_graph()
            ...
            if 'signed_request' in request.REQUEST:
                ...
                if request.facebook.signed_request.user.oauth_token:
                    social_user.update_facebook_token(request.facebook.signed_request)
...
class VkontakteAuthBackend(BaseSocialAuthBackend):
    def authenticate(self, request):
        ...
        social_user, created = SocialUserModel.objects.get_or_create(social_id=social_id,
                                                                     provider='vkontakte')
        if created:
